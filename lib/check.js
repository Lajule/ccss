  ...
  for (let declIndex = 0; declIndex < declarations.length; ++declIndex) {
    ...
    if (prevProperties.length > 0) {
      ...
      if (propOrder < currElement.order) {
        while (propOrder < currElement.order && rIndex < prevProperties.length) {
          rIndex += 1;
          currElement = prevProperties[declIndex - rIndex];
        }
        console.log(colors.magenta(`${source}:${start.line}: Property ${property} must be at line ${currElement.line}`));
        result = 1;
      }
    }
